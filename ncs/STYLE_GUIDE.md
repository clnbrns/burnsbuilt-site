...
<div className="flex items-start justify-between gap-3 mb-3">
  ...
  <div className="flex flex-col items-end text-right">
    <span className="text-[11px] text-ink-300 font-mono">{date}</span>
    <span className="scorenum text-[26px] leading-none mt-1">
      {need > 0 ? need : <Check size={20} className="text-win-500" />}
    </span>
...
**Body:** tournament name (`line-clamp-2` max), division mono code, progress bar (tone matches risk).

**CTAs (need > 0):** `Launch Campaign` (crimson) + `Invite Top Teams` (neutral). Row uses `flex gap-2`. The "Invite Top Teams" button needs `whitespace-nowrap` to prevent 3-line wrap inside narrow cards.
...
```
**Do not** use `grid-cols-5` or any column count > 3 — even if there are 5 brackets to show. With 5 cards per row, each card becomes ~200px wide and content collapses: tournament names truncate, header row stacks, "Invite Top Teams" button wraps to 3 lines. If there are more than 3 brackets, they wrap to the next row.
